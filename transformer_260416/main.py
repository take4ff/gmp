...
def prepare_data():
    ...
    if config.USE_DB:
        ...
        if not db_exists:
            ...
            force_print("[INFO] Run 'python -m transformer_260416.preprocess' first.")
        ...
        return train_loader, val_loader, test_loader, data_info, train, valid, test
...
def run_training(model, train_loader, val_loader, loss_fn, loss_wrapper,
    ...
    for epoch in range(config.EPOCHS):
        ...
        if early_stop_metric == "val_loss":
            ...
            # val_metrics は timestep ごとの辞書なので、全サンプルの加重平均を計算するか
            # 簡易的に最初のTSの値を参照するなどが必要。ここでは全体加重平均を計算
...
def main():
    ...
    # 1. データ準備
    train_loader, val_loader, test_loader, data_info, train, valid, test = prepare_data()
