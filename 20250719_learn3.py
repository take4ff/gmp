...
def custom_collate_fn(batch):
    ...
    if len(Y_batch_samples[0].shape) > 1:
...
class MutationPredictor(nn.Module):
    def __init__(self, embed_dim, vocab_size, sequence_length, max_co_occur, num_heads, num_encoder_layers, dropout_rate=0.1):
        ...
        self.position_embedding = nn.Embedding(sequence_length + 1, embed_dim)
    ...
    def forward(self, mutation_history_input, genome_input, target_pos=None):
        ...
        if target_pos is not None:
            ...
            for i in range(genome_input.shape[0]):
                ...
                end_idx = min(self.sequence_length, center_pos + half_window)
                ...
                if len(local_seq) < self.context_window_size:
                    ...
                    local_seq = local_seq[:self.context_window_size]
                
                local_genome_tensors.append(local_seq)
            
            local_genome_batch = torch.stack(local_genome_tensors)
            
            genome_emb_local = self.genome_base_embedding(local_genome_batch)
            genome_emb_local = genome_emb_local.permute(0, 2, 1)
            local_genome_summary_vector = self.local_genome_context_encoder(genome_emb_local).squeeze(-1)

        else:
            genome_emb_full = self.genome_base_embedding(genome_input)
            local_genome_summary_vector = genome_emb_full.mean(dim=1)
...
def train_model(model, train_loader, val_loader, criterion_orig, criterion_pos, criterion_new, optimizer, epochs, device, model_save_path):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (X_batch, Y_batch, genome_batch) in enumerate(train_loader):
            ...
            # 予測された位置を取得
            _, predicted_pos_idx = torch.max(pred_pos_logits_initial, 1)

            # 第2段階: 予測された位置のコンテキスト情報で最終予測
            pred_orig_logits, pred_pos_logits, pred_new_logits = model(X_batch, genome_batch, target_pos=predicted_pos_idx)
...
def evaluate_multi_label(model, test_dataloaders, device, model_path_to_load):
    ...
    with torch.no_grad():
        for strain_name, data_loader in test_dataloaders.items():
            ...
            all_precisions = []
            all_f1_scores = []
            all_hit_counts_in_top_n = []
            all_strict_match_accuracies = []
            ...
            for X_batch, Y_batch_true_raw, genome_batch in data_loader:
                ...
                for i in range(Y_batch_true_raw.shape[0]):
                    ...
                    n_true = len(true_mutations)
                    true_set = set(true_mutations)

                    all_possible_predictions = []
                    for orig_idx in range(model_config['vocab_size'] + 1):
                        for pos_idx in range(model_config['sequence_length'] + 1):
                            for new_idx in range(model_config['vocab_size'] + 1):
                    ...
                    if not all_possible_predictions:
                        all_recalls.append(0); all_precisions.append(0); all_f1_scores.append(0)
                        all_hit_counts_in_top_n.append(0); all_strict_match_accuracies.append(0)
                        continue
                    ...
                    num_correct_in_top_n = len(predicted_set & true_set)

                    recall = num_correct_in_top_n / n_true
