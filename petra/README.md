...
| 変異トークンのエンコード方式 | **v1（既定）が原論文のheadlineモデル(116M/458Mパラメータ)の方式と一致**。詳細は下記「訂正」節を参照 |

### 訂正（2026-07-15）: 変異トークンの再現対象を`version=0`→`version=1`に修正

原論文の実際のコード（`petra/PETra/data_preprocess/utils.py`の`mutation_encoding()`）を精読した結果、
**`version`引数で4種類のエンコード方式（0=9フィールド、1=フラット1トークン、2=4フィールド、3=7フィールド）が
存在し、論文Table 6/7で報告されているheadlineモデル(116M/458Mパラメータ)は`version=1`（フラット、
`(pos-1)*5+target_nuc+1`という式で決定、元の塩基(ref)は無視）を使っている**ことが判明した
（語彙内訳: 変異149,515=29,903位置×5終状態 + 位置情報366 + 時刻122 + 不明1 + 予約206 = 150,210で
論文Table 6と一致。version=0の語彙範囲(184,300超)とは不一致）。

以前の記述「v2は原論文の9フィールド(`version=0`)に倣った、より原論文に近い改良版」は誤りだった。
実際には**v1（フラット1トークン）の構造こそが原論文の実モデルと一致**しており、v2(6フィールド)は
論文のheadline結果とは無関係な別バリアント(`version=0`)の模倣に基づく拡張実験という位置づけが正しい。

**対応**: v1に残っていた1点の差分（元の塩基(ref)を保持している）を、`DROP_REF_BASE`フラグで
修正可能にした。`config.DROP_REF_BASE=True`にすると、`[mut]`トークンを`(position, 変異後塩基)`
のみで決定し、A1TとG1Tのように終状態が同じ変異は同一トークンになる（原論文`version=1`と同じ式）。
既定は`False`（既存挙動を維持、`20260708_145056`のwalk-forward結果はこの設定で得られたもの）。
`USE_REGION_FIELDS`とは直交フラグで、4通りの組み合わせをそれぞれ別キャッシュファイル
（`VOCAB_CACHE` / `VOCAB_CACHE_V2` / `VOCAB_CACHE_NOREF` / `VOCAB_CACHE_V2_NOREF`）で管理する。
