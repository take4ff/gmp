# --- dataset.py (Enhanced with Efficiency Features) ---
import torch
import numpy as np
...
# === 効率化機能クラス (transformer_251117からの移植) ===
class FeatureCache:
    """LRUキャッシュを使用した効率的な特徴量生成キャッシュ"""
    
    def __init__(self, max_size=None):
        self.max_size = max_size or config.CACHE_MAX_SIZE
    
    @lru_cache(maxsize=None)
    def get_cached_mutation_info(self, mutation, df_codon_hash, df_freq_hash, df_dissimilarity_hash):
        """変異情報をキャッシュ付きで取得"""
        # 実際の処理はprocess_single_mutationで実行
        return None
    
    def invalidate_cache(self):
        """キャッシュをクリア"""
        self.get_cached_mutation_info.cache_clear()
...
def get_dataframe_hash(df):
    """DataFrameのハッシュを計算してキャッシュキーとして使用"""
...
def process_feature_batch_optimized(df_batch, df_codon, df_freq, df_dissimilarity):
    ...
    # DataFrameのハッシュを事前計算
    if config.ENABLE_LRU_CACHE:
        df_codon_hash = get_dataframe_hash(df_codon)
        df_freq_hash = get_dataframe_hash(df_freq)
        df_dissimilarity_hash = get_dataframe_hash(df_dissimilarity)
    
    for _, row in df_batch.iterrows():
        try:
            path_mutations = row['path']
            strain = row['strain']
            original_len = row['original_len']
            
            sequence = []
            for mutations_str in path_mutations:
                timestep_features = Mutation_features(mutations_str, df_codon, df_freq, df_dissimilarity)
            ...
            if len(sequence) > 0:
                ...
                if len(last_timestep) > 0:
                    try:
                        # 正しい構造: last_timestep[0] は (cat_features, num_features) のタプル
                        cat_features = last_timestep[0][0]  # カテゴリカル特徴量
                        if len(cat_features) < 8:
                            force_print(f"[DEBUG] Categorical features too short: {len(cat_features)}, expected 8")
                            continue
                            
                        protein_id = cat_features[7]  # protein_token (8番目の要素)
                        pos_id = cat_features[1]      # base_pos (2番目の要素)
                    except (IndexError, TypeError) as ie:
                        force_print(f"[DEBUG] Structure error in last_timestep: {type(last_timestep)} with length {len(last_timestep)}")
                        if len(last_timestep) > 0:
                            force_print(f"[DEBUG] First item type: {type(last_timestep[0])}")
                            if hasattr(last_timestep[0], '__len__'):
                                force_print(f"[DEBUG] First item length: {len(last_timestep[0])}")
                        continue
                else:
                    continue
            else:
            ...
            batch_results.append({
                'sequence': sequence,
                'targets': (protein_id, pos_id),
                'original_len': original_len,
                'strain': strain,
                'raw_path': path_mutations
            })
            
        except Exception as e:
            force_print(f"[WARNING] Error processing row: {e}")
            continue
    
    return batch_results

def process_in_batches_parallel(df, batch_size, df_codon, df_freq, df_dissimilarity):
    """並列処理でバッチごとに特徴量生成"""
    if not config.ENABLE_PARALLEL_PROCESSING:
        ...
        for i in range(0, len(df), batch_size):
            ...
            results = process_feature_batch_optimized(batch, df_codon, df_freq, df_dissimilarity)
            all_results.extend(results)
            
            if (i // batch_size + 1) % 10 == 0:
                force_print(f"[INFO] Processed {i + len(batch)}/{len(df)} samples...")
        return all_results
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i in range(0, len(df), batch_size):
            batch = df.iloc[i:i+batch_size]
            future = executor.submit(process_feature_batch_optimized, batch, df_codon, df_freq, df_dissimilarity)
        ...
        for i, future in enumerate(futures):
            try:
                ...
                if (i + 1) % 10 == 0:
                    force_print(f"[INFO] Completed {i + 1}/{len(futures)} batches...")
            except Exception as e:
                force_print(f"[ERROR] Batch processing failed: {e}")
...
def get_mutation_data(names, lengths, paths, df_codon, df_freq, df_dissimilarity):
    """効率化された変異データ処理"""
    force_print(f"[INFO] Generating mutation features for {len(paths)} paths with optimization...")
    ...
    df_temp = pd.DataFrame({
        ...
        'path': paths,
        'strain': ['unknown'] * len(paths)  # strain情報がない場合のデフォルト
    })
    
    # バッチサイズを設定から取得
    batch_size = getattr(config, 'BATCH_SIZE_FEATURE_GEN', 1000)
    
    # 並列バッチ処理で特徴量生成
    start_time = time.time()
    all_results = process_in_batches_parallel(df_temp, batch_size, df_codon, df_freq, df_dissimilarity)
    end_time = time.time()
    
    force_print(f"[INFO] Feature generation completed in {end_time - start_time:.2f} seconds")
    force_print(f"[INFO] Generated features for {len(all_results)}/{len(paths)} paths")
    ...
    for result in all_results:
        processed_names.append(result.get('name', 'unknown'))
        processed_lengths.append(result['original_len'])
        features_paths.append(result['sequence'])
    
    return processed_names, processed_lengths, features_paths
...
def load_aux_data():
    """補助データ（コドン、頻度、類似度）を読み込む"""
    force_print("[INFO] Loading auxiliary data (codon, freq, dissimilarity)...")
...
class ViralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 先頭パディング: パディング部分が先頭、実データが末尾
        pad_len = config.SEQ_LEN - active_seq_len
        padding_mask[pad_len:] = False  # 実データ部分をFalseに設定
        
        # 先頭パディングのため、実データを末尾から配置
        for t, timestep_events in enumerate(sequence_to_pad):
            ...
            for c, (cat, num) in enumerate(timestep_events):
                ...
                padded_cat[target_idx, c] = cat  # target_idx を使用
                padded_num[target_idx, c] = num  # target_idx を使用
