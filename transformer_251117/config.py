...
MAX_STRAIN_NUM = 20  # 読み込むstrainの最大数 (Noneの場合は全strainを使用)
...
# --- 効率化設定 (transformer_251117からの移植) ---
ENABLE_EARLY_FEATURE_GENERATION = True  # 特徴量生成を分割前に実行するかどうか
BATCH_SIZE_FEATURE_GEN = 10000          # 特徴量生成時のバッチサイズ
ENABLE_LRU_CACHE = True                 # LRUキャッシュを使用するかどうか
ENABLE_PARALLEL_PROCESSING = True       # 並列処理を使用するかどうか
CACHE_MAX_SIZE = 10000                  # LRUキャッシュの最大サイズ
EARLY_STOPPING_PATIENCE = 5             # Early Stoppingのpatience
...
MODEL_SAVE_DIR = './models/transformer_251117/'  # モデルの保存ディレクトリ