...
class HierarchicalTransformer(nn.Module):
    def __init__(self):
        ...
        if getattr(config, 'USE_HOMOPLASY_PRIOR', False):
            bias_vec = self._load_homoplasy_bias()
    ...
    def _load_homoplasy_bias(self):
        """提案10: 位置別再発回数 CSV から log(1+r) バイアスベクトルを構築する。

        CSV は 'position_id','recurrence_count' 列を持つ。存在しない/読めない場合は
        None を返し、ホモプラシー事前分布を無効化する（現行挙動と同一）。
        """
        import os
        path = getattr(config, 'HOMOPLASY_CSV', '')
        if not path or not os.path.exists(path):
            _log.force_print(f"[WARN] USE_HOMOPLASY_PRIOR=True but CSV not found: {path}. Prior disabled.")
            return None
        try:
            import pandas as pd
            df = pd.read_csv(path)
            ...
            _log.force_print(f"[INFO] Homoplasy prior loaded: {len(df)} sites from {path}")
            return bias
        except Exception as e:
            _log.force_print(f"[WARN] Failed to load homoplasy CSV ({path}): {e}. Prior disabled.")
            return None
