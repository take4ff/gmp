...
Usage:
    ...
    python -m transformer_260707.scripts.eval.optuna_search --n-trials 30
    # 中断後の再開: 同じ --study-name / --storage を指定すれば続きから
    # 特定パラメータのみ探索: --params lr dropout

出力:
    outputs/transformer_260707/scripts/optuna/<study_name>.db          （SQLite: 全 trial 履歴）
    outputs/transformer_260707/scripts/optuna/<study_name>_best.json    （ベスト構成）
...
def _make_objective(params):
    def objective(trial):
        import optuna
        from transformer_260707 import config
        from transformer_260707.main import main
...
def main_cli():
    ...
    out_dir = os.path.join('outputs', 'transformer_260707', 'scripts', 'optuna')
