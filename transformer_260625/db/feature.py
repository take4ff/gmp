...
def preprocess_static_data(df_codon, df_freq, df_dissimilarity, df_pam250, 
    ...
    codon_data = {
        ...
        'aa_pos': df_codon['protein_pos'].tolist(),
...
def Feature_from_csv_fast(mutation, codon_state, freq_dict, dissim_dict, pam250_dict,
    ...
    if bef == base and codon != "none":
        if undo_list is not None:
            undo_list.append(('base', idx, codon_state['base'][idx]))
    ...
    def __getitem__(self, idx):
        ...
        for t, timestep_events in enumerate(sequence_to_pad):
            ...
            for c, (cat_origin, num_origin) in enumerate(timestep_events):
                ...
                # カテゴリカル特徴量の個別マスク (1文字ずつまたは一括)
                if config.ABLATION_MASKS.get('MUTATION_CONTEXT', False):
                    for i in range(9, 15):
                        cat[i] = 0
                else:
                    if config.ABLATION_MASKS.get('MUTATION_CONTEXT_L3', False):
                        cat[9] = 0
                    if config.ABLATION_MASKS.get('MUTATION_CONTEXT_L2', False):
                        cat[10] = 0
                    if config.ABLATION_MASKS.get('MUTATION_CONTEXT_L1', False):
                        cat[11] = 0
                    if config.ABLATION_MASKS.get('MUTATION_CONTEXT_R1', False):
                        cat[12] = 0
                    if config.ABLATION_MASKS.get('MUTATION_CONTEXT_R2', False):
                        cat[13] = 0
                    if config.ABLATION_MASKS.get('MUTATION_CONTEXT_R3', False):
                        cat[14] = 0
