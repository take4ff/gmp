...
def evaluate_accuracy(model, dataloader, device):
    """
    テストデータでのモデルの予測精度を正解率で評価する関数。

    Args:
        model: 評価するPyTorchモデル。
        dataloader: テストデータのDataLoader。
        device: モデルを配置するデバイス (例: 'cuda', 'cpu')。
...
def evaluate_mutation_part_accuracy(model, dataloader, device, tokenizer):
    """
    テストデータでのモデルの予測精度を変異前塩基、変異位置、変異後塩基ごとに評価する関数。
    MutationDatasetForLM のデータ構造をそのまま使用。

    Args:
        model: 評価するPyTorchモデル。
        dataloader: テストデータのDataLoader (Dataset は MutationDatasetForLM を想定)。
        device: モデルを配置するデバイス (例: 'cuda', 'cpu')。
        tokenizer: 使用しているトークナイザー。
    ...
    """
    model.eval()
    total_correct_base_before = 0
    total_correct_position = 0
    total_correct_base_after = 0
    total_samples = 0
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Evaluating'):
            ...
            for i in range(len(input_ids)):
                # 予測されたラベルの位置を取得 (最後の非パディング位置)
                masked_indices = (labels[i] != -100).nonzero(as_tuple=True)[0]
                if masked_indices.numel() > 0:
                    predicted_index = masked_indices[-1].item()
                    predicted_token_id = predictions[i][predicted_index]
                    predicted_token_str = tokenizer.decode(predicted_token_id)

                    # 正解ラベルのトークンIDを取得
                    label_token_id = labels[i][predicted_index].item()
                    label_token_str = tokenizer.decode(label_token_id)

                    if len(predicted_token_str) > 0 and len(label_token_str) > 0:
                        if len(predicted_token_str) >= 1 and len(label_token_str) >= 1 and predicted_token_str[0] == label_token_str[0]:
                            total_correct_base_before += 1
                        if len(predicted_token_str) >= 2 and len(label_token_str) >= 2 and predicted_token_str[1:-1] == label_token_str[1:-1]:
                            total_correct_position += 1
                        if len(predicted_token_str) >= 1 and len(label_token_str) >= 1 and predicted_token_str[-1] == label_token_str[-1]:
                            total_correct_base_after += 1
                        total_samples += 1

    accuracy_base_before = total_correct_base_before / total_samples if total_samples > 0 else 0
    accuracy_position = total_correct_position / total_samples if total_samples > 0 else 0
    accuracy_base_after = total_correct_base_after / total_samples if total_samples > 0 else 0

    return accuracy_base_before, accuracy_position, accuracy_base_after


test1_datasetForLM = MutationDatasetForLM(X_test1, y_test1, tokenizer, MAX_LEN)
test1_dataloader = DataLoader(test1_datasetForLM, batch_size=BATCH_SIZE)

accuracy = evaluate_accuracy(model, test1_dataloader, device)
print(f'Test Accuracy: {accuracy:.4f}')
accuracy_base_before, accuracy_position, accuracy_base_after = evaluate_mutation_part_accuracy(model, test1_dataloader, device, tokenizer)
print(f'Test Accuracy (Base Before): {accuracy_base_before:.4f}')
print(f'Test Accuracy (Position): {accuracy_position:.4f}')
print(f'Test Accuracy (Base After): {accuracy_base_after:.4f}')


test2_datasetForLM = MutationDatasetForLM(X_test2, y_test2, tokenizer, MAX_LEN)
test2_dataloader = DataLoader(test2_datasetForLM, batch_size=BATCH_SIZE)

accuracy = evaluate_accuracy(model, test2_dataloader, device)
print(f'Test Accuracy: {accuracy:.4f}')
accuracy_base_before, accuracy_position, accuracy_base_after = evaluate_mutation_part_accuracy(model, test2_dataloader, device, tokenizer)
print(f'Test Accuracy (Base Before): {accuracy_base_before:.4f}')
print(f'Test Accuracy (Position): {accuracy_position:.4f}')
print(f'Test Accuracy (Base After): {accuracy_base_after:.4f}')


test3_datasetForLM = MutationDatasetForLM(X_test3, y_test3, tokenizer, MAX_LEN)
test3_dataloader = DataLoader(test3_datasetForLM, batch_size=BATCH_SIZE)

accuracy = evaluate_accuracy(model, test3_dataloader, device)
print(f'Test Accuracy: {accuracy:.4f}')
accuracy_base_before, accuracy_position, accuracy_base_after = evaluate_mutation_part_accuracy(model, test3_dataloader, device, tokenizer)
print(f'Test Accuracy (Base Before): {accuracy_base_before:.4f}')
print(f'Test Accuracy (Position): {accuracy_position:.4f}')
print(f'Test Accuracy (Base After): {accuracy_base_after:.4f}')
