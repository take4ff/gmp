...
import module.save2_graphit as save
...
# 8. ✅ GraphiT特有の分析結果保存
try:
    save.save_graphit_analysis(model, train_loader, val_loader, save_dir, device)
    save.save_attention_weights_analysis(model, val_loader, save_dir, device)
    print("✅ GraphiT分析結果の保存完了")
except Exception as e:
    print(f"⚠️ GraphiT分析結果の保存中にエラー: {e}")

# 9. ✅ 混同行列の保存
try:
    save.save_confusion_matrix(model, val_loader, train_dataset.label_encoder, device, save_dir)
    print("✅ 混同行列の保存完了")
except Exception as e:
    print(f"⚠️ 混同行列の保存中にエラー: {e}")
