...
class PretrainedBERTModel(nn.Module):
    def __init__(self, bert_model_name, num_classes, mutation_vocab, freeze_layers=0):
        ...
        try:
            ...
            vocab_mapping_tensor = torch.full(
                (max_custom_id + 1,), 
            ...
            for custom_id, bert_id in self.vocab_mapping.items():
                vocab_mapping_tensor[custom_id] = bert_id
                
            # テンソルをパラメータとして登録（GPUに自動移動）
            # 既に存在するかチェックして、存在しない場合のみ登録
            if not hasattr(self, 'vocab_mapping_tensor'):
                self.register_buffer('vocab_mapping_tensor', vocab_mapping_tensor)
            else:
                # 既に存在する場合は値を更新
                self.vocab_mapping_tensor = vocab_mapping_tensor
            print("高速変換テーブル作成完了")
