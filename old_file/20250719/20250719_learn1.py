...
try:
    ...
    GLOBAL_GENOME_TENSOR = torch.full((model_config['sequence_length'],), PAD_BASE, dtype=torch.long) # ダミーデータ
...
def add_x_by_y(x, y):
    ...
    for i in range(len(y)):
        # y[i] は [['G', '400', 'A']] または [['C', '200', 'G'], ['T', '300', 'A']] のような形式
        for fea in y[i]: # fea は ['G', '400', 'A'] のような単一変異リスト
            new_x.append(x[i])
            new_y.append(fea) 
    return new_x, new_y
...
class MutationPathDataset(Dataset):
    ...
    def _hgvs_to_int(self, mutation_list_input):
        ...
        """
        # データが [['C', '15279', 'T']] のようにネストされている場合にも対応
    ...
    def _process_hgvs_paths_Y(self, hgvs_paths_list, is_test_data):
        ...
        for y_element in hgvs_paths_list:
            if is_test_data:
                ...
                # 訓練/検証データはadd_x_by_yにより ['C', '15279', 'T'] の形式になる想定
                # もし [['C', '15279', 'T']] のようにリストでラップされていれば、_hgvs_to_int内で処理
...
class MutationPredictor(nn.Module):
    ...
    def forward(self, mutation_history_input, genome_input, target_pos=None):
        ...
        co_mutation_context = self.history_positional_encoding(co_mutation_context.permute(1, 0, 2)).permute(1, 0, 2)
        ...
        if target_pos is not None: # 訓練/検証時: ターゲット位置が与えられる
            ...
            for i in range(genome_input.shape[0]):
                ...
                if len(local_seq) < self.context_window_size:
                    ...
                    local_seq = local_seq[:self.context_window_size] # 必要に応じて切り捨て
            ...
            genome_emb_local = genome_emb_local.permute(0, 2, 1) # (batch_size, embed_dim, context_window_size)
...
def evaluate_multi_label(model, test_dataloaders, device):
    ...
    with torch.no_grad():
        for strain_name, data_loader in test_dataloaders.items():
            ...
            avg_hit_count_in_top_n = np.mean(all_hit_counts_in_top_n)
            avg_strict_match_accuracy = np.mean(all_strict_match_accuracies)
