...
import module.mutation_itransformer as mit
...
# モデルのインスタンス化
model = mit.MutationITransformer(
    ...
    feature_mask=base_features_mask
).to(device)
...
try:
    for epoch in range(num_epochs):
        ...
        # 訓練
        train_loss, train_acc = mit.train_epoch(model, train_loader, criterion, optimizer, device)
        
        # 検証
        val_loss, val_acc, val_preds, val_targets = mit.evaluate(model, val_loader, criterion, device)
...
# 2. 設定の保存
config = save.save_hyperparameters_and_config(
    model=model,
