# --- evaluate.py --- (transformer_260417)
import torch
...
def evaluate(model, dataloader, loss_fn, strength_thresholds=None):
    ...
    # ECE 計算用: タイムステップ別に (pred_probs, hit) のリストを蝉積
    ...
    with torch.no_grad():
        for (x_cat, x_num, mask), y_batch, batch_lens, batch_strains, batch_strength_scores, batch_input_strs, batch_target_strs, batch_full_paths, raw_y_batch in dataloader:
            ...
            # サンプルごとのメトリクス集計
            # [260417] Soft Target モードでは raw_y_batch（全ルートのフラットターゲット）を評価に使用
...
def evaluate_topk(model, dataloader, ks=(1, 3, 5)):
    ...
    with torch.no_grad():
        for (x_cat, x_num, mask), _y_batch, _, _, _, _, _, _, raw_y_batch in dataloader:
            x_cat = x_cat.to(config.DEVICE)
            x_num = x_num.to(config.DEVICE)
            mask  = mask.to(config.DEVICE)
