# --- utils/losses.py --- (transformer_260417)
# 損失計算の共通ロジック (train.py / evaluate.py 共用)
# [260417] Soft Target（確率分配ベクトル）対応版
...
def build_loss_fn(class_counts_dict=None):
    ...
    [260417] FocalLoss が Soft Target に対応（アプローチA）したため再度使用可能。
             Soft Target（float ベクトル target）は PyTorch の CrossEntropyLoss でも
             FocalLoss でも同じ I/F (FloatTensor [N, C]) で渡せる。
    ...
    _log.force_print(
        f"[INFO] (260417) Using Loss Function: {loss_type.upper()} "
