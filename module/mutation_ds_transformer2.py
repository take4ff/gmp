...
class MutationDSTransformer(nn.Module):
    ...
    def forward(self, categorical_x, count_x, padding_mask=None):
        ...
        # Stream 2: 特徴量アテンション（修正版）
        # 各特徴量を個別に処理
        feature_outputs = []
        for i, embedded in enumerate(embedded_features):
            # embedded は [batch, seq_len, embed_dim] なので、そのまま使用
            feature_out = self.feature_encoder(embedded)  # 修正: 次元操作を削除
        ...
        # 融合: 単純な結合
        combined = torch.cat([temporal_pooled, feature_pooled], dim=-1)
