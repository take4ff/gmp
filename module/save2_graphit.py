...
def analyze_attention_patterns(model, dataloader, device):
    ...
    try:
        model.eval()
        attention_stats = {
            "model_type": "MutationGraphiT",
            "num_attention_heads": getattr(model, 'actual_nhead', 8),
            "num_layers": getattr(model, 'actual_num_layers', 4),
            "analysis_status": "completed"
        }
        
        # 詳細分析を試みる（エラー時は基本情報のみ）
        try:
            sample_batch = next(iter(dataloader))
            categorical_data = sample_batch['categorical'][:1].to(device)
            
            with torch.no_grad():
                attention_stats["sample_sequence_length"] = categorical_data.shape[-1]
                attention_stats["batch_size_analyzed"] = 1
                
        except Exception as inner_e:
            attention_stats["detailed_analysis"] = f"スキップ: {str(inner_e)}"
...
def save_graphit_analysis(model, train_loader, val_loader, save_dir, device):
    """GraphiT特有の分析結果を保存"""
    import json
    
    print("GraphiT分析結果を保存中...")
    
    # 1. グラフ構造分析
    graph_stats = analyze_graph_structure(model, val_loader, device)
    
    # 2. アテンション重み分析
    attention_analysis = analyze_attention_patterns(model, val_loader, device)
    
    # 3. 変異ネットワーク可視化
    save_mutation_network_visualization(model, val_loader, save_dir, device)
    ...
    analysis_path = os.path.join(save_dir, "graph_analysis.json")
    with open(analysis_path, 'w', encoding='utf-8') as f:
        json.dump(graph_analysis_results, f, indent=2, ensure_ascii=False)
    
    print(f"GraphiT分析結果を保存: {analysis_path}")
    return graph_analysis_results

def analyze_graph_structure(model, dataloader, device):
    """グラフ構造の統計分析"""
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # グラフ構造を生成
            adj_matrices = model.create_mutation_graph(categorical_data, count_data)
            
            # バッチごとの統計を計算
            for adj_matrix in adj_matrices:
                total_connections = (adj_matrix > 0).sum().item()
                seq_len = adj_matrix.shape[0]
                density = total_connections / (seq_len * seq_len)
                
                connectivity_stats["total_connections"].append(total_connections)
                connectivity_stats["density"].append(density)
    
    # 統計サマリーを計算
    return {
        "avg_connections": np.mean(connectivity_stats["total_connections"]),
        "avg_density": np.mean(connectivity_stats["density"]),
        "std_connections": np.std(connectivity_stats["total_connections"]),
        "std_density": np.std(connectivity_stats["density"])
    }

def save_mutation_network_visualization(model, dataloader, save_dir, device):
    """変異ネットワークの可視化"""
    model.eval()
    
    # サンプルデータを取得
    sample_batch = next(iter(dataloader))
    categorical_data = sample_batch['categorical'][:1].to(device)  # 1サンプルのみ
    count_data = sample_batch['count'][:1].to(device)
    
    with torch.no_grad():
        adj_matrix = model.create_mutation_graph(categorical_data, count_data)[0]
    ...
    plt.title('Mutation Network Structure (Sample)')
    plt.axis('off')
    
    network_path = os.path.join(save_dir, "mutation_network_visualization.png")
    plt.savefig(network_path, dpi=300, bbox_inches='tight')
    print(f"変異ネットワーク可視化を保存: {network_path}")
    plt.close()

def save_attention_weights_analysis(model, dataloader, save_dir, device):
    """アテンション重みの分析と可視化"""
    model.eval()
    ...
    # フォワードパスでアテンション重みを取得
    with torch.no_grad():
        x = model.embed_features(categorical_data, count_data)
        adj_matrix = model.create_mutation_graph(categorical_data, count_data)
        
        attention_weights = []
        for layer in model.graphit_layers:
            # アテンション重みを取得（実装に応じて調整が必要）
    ...
    for i, attn in enumerate(attention_weights[:4]):  # 最初の4層
        sns.heatmap(attn, ax=axes[i], cmap='viridis', 
                   xticklabels=False, yticklabels=False)
        axes[i].set_title(f'Layer {i+1} Attention Weights')
    
    plt.tight_layout()
    attention_path = os.path.join(save_dir, "attention_weights_analysis.png")
    plt.savefig(attention_path, dpi=300, bbox_inches='tight')
    print(f"アテンション重み分析を保存: {attention_path}")
    plt.close()