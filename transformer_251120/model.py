...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, loss1, loss2):
        # loss1: Region Loss, loss2: Position Loss
        
        # 公式: Loss = exp(-s) * L + s  (s = log_variance)
        
        # Task 1 (Region)
        precision1 = torch.exp(-self.log_vars[0])
        loss_sum = precision1 * loss1 + self.log_vars[0]
        
        # Task 2 (Position)
        precision2 = torch.exp(-self.log_vars[1])
        loss_sum += precision2 * loss2 + self.log_vars[1]
