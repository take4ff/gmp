...
def separate_XY(feature_paths, original_lengths, raw_paths, strain_names, max_x_len, ylen=1):
    """
    シーケンスを特徴量(X)とラベル(Y)に分割し、メタデータ(Strain等)も保持する
    """
    x_y_len_list = []
    ...
    for item, original_len, raw_path, strain in zip(feature_paths, original_lengths, raw_paths, strain_names):
        if len(item) > ylen:
            ...
            # Strain情報もタプルに追加
            x_y_len_list.append((x, y_targets, original_len, raw_path, strain))
...
class ViralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sequence, targets, original_len, raw_path, strain = self.data[idx]
        ...
        for t, timestep_events in enumerate(sequence_to_pad):
            ...
            for c, (cat_origin, num_origin) in enumerate(timestep_events):
                ...
                # 3. タンパク質位置のマスク
                if config.ABLATION_MASKS['PROTEIN_POS']:
                    cat[5] = 0 # PAD (または0が有効値なら特定の無効ID)
...
def create_dataloader(data, batch_size, shuffle=True):
    ...
    def collate_fn(batch):
        ...
        # 戻り値の順序を変更・追加
        return (batch_x_cat, batch_x_num, batch_mask), batch_y, batch_lens, batch_strains, batch_input_strs, batch_target_strs, batch_full_paths
