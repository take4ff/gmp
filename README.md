...
* **カテゴリ**: `【評価・検証機構】`
* **内容**: 現在の固定日付・固定タイムステップ分割に代わり、月単位または四半期単位で学習→予測を繰り返す時系列交差検証を実施する。各時点で「それ以前の全データで学習し、直後の期間を予測する」ため、実際の運用シナリオを忠実に再現でき、単一分割では捉えられない時期ごとの精度変動を計測できる。
* **採用方式**: **展開ウィンドウ（Expanding Window）** — 学習データを毎 fold 累積していく方式を採用する。変異の進化系譜（Ancestral → Delta → Omicron）が後の変異予測に寄与するため、過去の全履歴を活用する展開ウィンドウが理論的に適している。

  | 方式 | 概要 | 特性 |
  |---|---|---|
  | **展開ウィンドウ** ✅ | 学習データを fold ごとに累積（過去全件使用） | 進化履歴を最大限活用。採用方式。 |
  | スライディングウィンドウ | 直近 N ヶ月のみで学習（窓を固定幅でスライド） | 古いデータを排除し直近トレンドへ追従。オミクロン以降への特化などに有効。 |
  ...
  Fold1: Train ─2020/12  Test: 2021/01-06  (安定期: Alpha)
  Fold2: Train ─2021/06  Test: 2021/07-12  ← Omicron出現を含む移行期
  Fold3: Train ─2021/12  Test: 2022/01-06  (安定期: Omicron BA.1/2)
  Fold4: Train ─2022/06  Test: 2022/07-12  (安定期: BA.4/5)
  Fold5: Train ─2022/12  Test: 2023/01-06  (安定期: XBB)
  Fold6: Train ─2023/06  Test: 2023/07-12  ← JN.1出現を含む移行期
Fold7: Train ─2023/12  Test: 2024/01-06  (安定期: JN.1)
  ```
