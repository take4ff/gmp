...
def evaluate(model, dataloader, loss_fn, strength_thresholds=None):
    ...
    results_by_timestep = defaultdict(lambda: {
        ...
        "preds_strength": [], "targets_strength": []  # 流行度予測
    })
    
    # 流行度カテゴリ別 (timestep, category) -> {preds_strength, targets_strength, ...}
    ...
    with torch.no_grad():
        ...
        for (x_cat, x_num, mask), y_batch_list, batch_lens, batch_strains, batch_strength_scores, batch_input_strs, batch_target_strs, batch_full_paths in dataloader:
            ...
            topk_indices_aa_pos = torch.topk(predictions_aa_pos, config.TOP_K_EVAL, dim=1).indices
            topk_indices_codon_pos = torch.topk(predictions_codon_pos, config.TOP_K_EVAL, dim=1).indices
            topk_indices_synonymous = torch.topk(predictions_synonymous, config.TOP_K_EVAL, dim=1).indices
            ...
            for i in range(len(y_batch_list)):
                ...
                detailed_results.append({
                    ...
                    'targets_aa_pos': target_set_aa_pos,
                    'preds_aa_pos': pred_set_aa_pos,
                    'hit_aa_pos': hit_aa_pos,
                    'targets_codon_pos': target_set_codon_pos,
                    'preds_codon_pos': pred_set_codon_pos,
                    'hit_codon_pos': hit_codon_pos,
                    'targets_synonymous': target_set_synonymous,
                    'preds_synonymous': pred_set_synonymous,
                    'hit_synonymous': hit_synonymous
                })
                ...
                targets_aa_pos_tensor = torch.tensor([t[2] for t in targets_tuples], dtype=torch.long).to(config.DEVICE)
                targets_codon_pos_tensor = torch.tensor([t[3] for t in targets_tuples], dtype=torch.long).to(config.DEVICE)
                targets_synonymous_tensor = torch.tensor([t[4] for t in targets_tuples], dtype=torch.long).to(config.DEVICE)
                ...
                loss_aa_pos = loss_fn(predictions_aa_pos[i].expand(num_targets, -1), targets_aa_pos_tensor)
                loss_codon_pos = loss_fn(predictions_codon_pos[i].expand(num_targets, -1), targets_codon_pos_tensor)
                loss_synonymous = loss_fn(predictions_synonymous[i].expand(num_targets, -1), targets_synonymous_tensor)
