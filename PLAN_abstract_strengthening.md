...
合計701字（149字の余裕あり）。デルタ/オミクロンの数値はユーザ判断で現状のまま維持。

新型コロナウイルスの変異株は短期間で出現し感染拡大を招くが、既存のゲノム配列全長を用いる変異予測手法は計算コストが高く、予測結果の説明性にも課題があった。

本研究では、ゲノム全長ではなく変異履歴（変異前塩基・位置・変異後塩基の出現順系列）を入力とし、各変異にアミノ酸性質・変異頻度・コドン組成等の生物学的特徴量を付加したTransformerベースの変異予測モデルを提案する。ほぼ同時に生じたとされる変異群を集約し代表ベクトルを生成する層の導入により、変異発生順序の非確実性に対応した。このアーキテクチャによって変異履歴の構造理解と特徴抽出を実現し、実際にAttention重みはD614G等の既知系統定義変異へ時期を通じて一貫して高い重みを割り当てていることを確認した。

半年おきのウォークフォワードテストで変異パス末尾を予測した結果、デルタ株で遺伝子領域45%・塩基配列位置35%、オミクロン株で遺伝子領域50%・位置10%の精度を達成し、いずれも一様ランダム予測（領域2.7%・位置0.003%）を大きく上回った。位置精度の低下はオミクロン株の変異多様化に起因する一方、遺伝子レベルの蓄積傾向（Spike遺伝子領域への集中等）はモデルが引き続き捉えていることが確認された。シノニマス変異・コドン中の位置の予測は一貫して各80%・60%程度の精度となった。共起集約を持たないPETRA型ベースラインとの比較でも優位性を確認した。

今後は遺伝子領域→シノニマス性→コドン位置の順に候補を絞り込む段階的予測を検討し、塩基配列位置の精度向上を検証する。本手法により変異動態を捉え、早期感染対策への貢献を目指す。

**まだプレースホルダーの箇所**（実データ確定後に更新要）:
- 「遺伝子レベルの蓄積傾向（Spike遺伝子領域への集中等）」→ genome_track実行待ち（Phase C, #9）
- 「PETRA型ベースラインとの比較でも優位性を確認した」→ PETRA実学習・比較待ち（Phase E/F, #7/#11）。優位性が確認できなければ「同等以下のコストで匹敵する性能」等に書き換え要
