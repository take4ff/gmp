# --- transformer_260723/scripts/feature_importance.py ---
...
Usage:
    python -m transformer_260723.scripts.feature_importance \\
...
def collect_coattn_weights(model, loader, n_batches, device):
    ...
    def _patched(x, co_occur_mask=None, need_weights=False, **kwargs):
        # **kwargs は freq_penalty（USE_COATTN_FREQUENCY_PENALTY有効時のみ渡される）を素通し
        out, w = original_forward(x, co_occur_mask=co_occur_mask, need_weights=True, **kwargs)
        # w: [B*T, 1, C]
...
def main():
    ...
    # 実行時オーバーライドを反映しない静的コピーのため（save_config_copy()参照）、
    # 何もしないとTEMPORAL_SPLIT_DATE/USE_POINT_IN_TIME_FREQが学習時と食い違い、
    # 静的FREQ_CSV由来の未来リークがcodon_freq(x_num[...,0])に再混入する
    # （2026-07-29 codon_freq高重要度の原因調査より）。
    import re as _re
