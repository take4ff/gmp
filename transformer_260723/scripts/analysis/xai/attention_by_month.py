...
# 2 つのモード:
#   (A) 単一チェックポイント:
#     python -m transformer_260723.scripts.analysis.xai.attention_by_month \
...
#     python -m transformer_260723.scripts.analysis.xai.attention_by_month \
#         --walk_forward_dir outputs/transformer_260723/results/walk_forward/20260706_223816
import argparse
import os
import re
...
def _attach_attention_capture(model):
    ...
    def wrapped(x, co_occur_mask=None, need_weights=False, **kwargs):
        # **kwargs は freq_penalty（USE_COATTN_FREQUENCY_PENALTY有効時のみHierarchical
        # Transformer.forwardから渡される）をそのまま素通しする。無効時は空で無害。
        out, w = orig_forward(x, co_occur_mask=co_occur_mask, need_weights=True, **kwargs)
        B, T, C, _ = x.shape
